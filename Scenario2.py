...
def spawn_pedestrians_around_ego_vehicles(ego_vehicle, radius, spawn_points, number_of_pedestrians, world, client):
    ...
    # Get the vehicle waypoint
    vehicle_location = ego_vehicle.get_location()
    world_map = world.get_map()
    vehicle_waypoint = world_map.get_waypoint(vehicle_location)

    # Move one lane to the right until the sidewalk is found
    lane_type = None
    while lane_type != carla.LaneType.Sidewalk:  
        vehicle_right_waypoint = vehicle_waypoint.get_right_lane()
        lane_type = vehicle_right_waypoint.lane_type
...
def spawn_bikes(ego_vehicle, radius, spawn_points, number_of_bikes, world, client):
    ...
    if len(accessible_points) < number_of_bikes:
        # if your radius is relatively small,the satisfied points may be insufficient
        number_of_bikes = len(accessible_points)
...
class World(object):
    ...
    def restart(self):
        ...
        # spawn more vehicles
        #spawn_points = self.map.get_spawn_points()
        #spawn_vehicles_around_ego_vehicles(self.player, radius=60, spawn_points=spawn_points, numbers_of_vehicles=80, world=self.world, client=self.client)
